...
func LoadConfig(path string) (*Config, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			logger.WarnF(
				"config file not found, using default config with env overrides",
				map[string]any{"path": path},
			)
			cfg := DefaultConfig()
			_ = env.Parse(cfg)
			applySkillsRegistryEnvCompat(cfg)
			applyDynamicEnvironmentOverrides(cfg)
			if initErr := InitChannelList(cfg.Channels); initErr != nil {
				return nil, initErr
			}
...
func applyDynamicEnvironmentOverrides(cfg *Config) {
	if cfg == nil {
		return
	}

	// 1. Render Port and Host overrides
	// Render provides $PORT for web services. If PICOCLAW_GATEWAY_PORT is not explicitly set,
	// bind to Render's $PORT.
	if portStr := os.Getenv("PORT"); portStr != "" {
		if os.Getenv("PICOCLAW_GATEWAY_PORT") == "" {
			if port, err := strconv.Atoi(strings.TrimSpace(portStr)); err == nil && port > 0 {
				cfg.Gateway.Port = port
			}
		}
		if os.Getenv("PICOCLAW_GATEWAY_HOST") == "" && (cfg.Gateway.Host == "" || cfg.Gateway.Host == "localhost") {
			cfg.Gateway.Host = "0.0.0.0"
		}
	}
	if os.Getenv("RENDER") != "" && os.Getenv("PICOCLAW_GATEWAY_HOST") == "" {
		cfg.Gateway.Host = "0.0.0.0"
	}

	// 2. Dynamic Custom AI Model via environment variables
	customKey := strings.TrimSpace(os.Getenv("CUSTOM_MODEL_API_KEY"))
	if customKey == "" {
		customKey = strings.TrimSpace(os.Getenv("MODEL_API_KEY"))
	}
	customBase := strings.TrimSpace(os.Getenv("CUSTOM_MODEL_BASE_URL"))
	if customBase == "" {
		customBase = strings.TrimSpace(os.Getenv("CUSTOM_MODEL_ENDPOINT"))
	}
	customName := strings.TrimSpace(os.Getenv("CUSTOM_MODEL_NAME"))
	customProvider := strings.TrimSpace(os.Getenv("CUSTOM_MODEL_PROVIDER"))
	customModelID := strings.TrimSpace(os.Getenv("CUSTOM_MODEL_IDENTIFIER"))

	if customKey != "" || customBase != "" || customName != "" {
		if customName == "" {
			customName = "custom-model"
		}
		if customModelID == "" {
			customModelID = customName
		}
		if customProvider == "" {
			customProvider = "openai"
		}

		var secureKeys SecureStrings
		if customKey != "" {
			secureKeys = SecureStrings{NewSecureString(customKey)}
		}
		...
		if tf := strings.TrimSpace(os.Getenv("CUSTOM_MODEL_TOOL_SCHEMA_TRANSFORM")); tf != "" {
			modelEntry.ToolSchemaTransform = tf
		}
		if tl := strings.TrimSpace(os.Getenv("CUSTOM_MODEL_THINKING_LEVEL")); tl != "" {
			modelEntry.ThinkingLevel = tl
		}

		found := false
		for i, m := range cfg.ModelList {
			if m != nil && m.ModelName == customName {
				cfg.ModelList[i] = modelEntry
				found = true
				break
			}
		}
		...
		if rawFallbacks := strings.TrimSpace(os.Getenv("CUSTOM_MODEL_FALLBACKS")); rawFallbacks != "" {
			...
			for _, fb := range parts {
				...
				if fb != "" {
					cleanFallbacks = append(cleanFallbacks, fb)
			...
			cfg.Agents.Defaults.ModelFallbacks = cleanFallbacks
		}
	...
	if tgToken != "" {
		if os.Getenv("PICOCLAW_CHANNELS_TELEGRAM_TOKEN") == "" {
			_ = os.Setenv("PICOCLAW_CHANNELS_TELEGRAM_TOKEN", tgToken)
		}
		if cfg.Channels == nil {
			cfg.Channels = make(ChannelsConfig)
		}
		ch := cfg.Channels[ChannelTelegram]
		if ch == nil {
			ch = &Channel{
				Type:    ChannelTelegram,
				Enabled: true,
			}
			ch.SetName(ChannelTelegram)
			cfg.Channels[ChannelTelegram] = ch
		} else {
			ch.Enabled = true
		}

		var tgSettings *TelegramSettings
		if decoded, err := ch.GetDecoded(); err == nil && decoded != nil {
			if s, ok := decoded.(*TelegramSettings); ok {
				tgSettings = s
			}
		}
		if tgSettings == nil {
		...
		tgSettings.Token = *NewSecureString(tgToken)
		if base := strings.TrimSpace(os.Getenv("TELEGRAM_BASE_URL")); base != "" {
			tgSettings.BaseURL = base
		}
		if proxy := strings.TrimSpace(os.Getenv("TELEGRAM_PROXY")); proxy != "" {
			tgSettings.Proxy = proxy
		}
		if rawJSON, err := json.Marshal(tgSettings); err == nil {
			ch.Settings = rawJSON
		}
		ch.extend = tgSettings

		allowedUsers := strings.TrimSpace(os.Getenv("TELEGRAM_ALLOWED_USERS"))
		if allowedUsers == "" {
			allowedUsers = strings.TrimSpace(os.Getenv("PICOCLAW_CHANNELS_TELEGRAM_ALLOW_FROM"))
		}
		if allowedUsers != "" {
			users := strings.Split(allowedUsers, ",")
			trimmed := make([]string, 0, len(users))
			for _, u := range users {
				if tu := strings.TrimSpace(u); tu != "" {
					trimmed = append(trimmed, tu)
				}
			}
			if len(trimmed) > 0 {
				ch.AllowFrom = trimmed
			}
		}
	}
}
