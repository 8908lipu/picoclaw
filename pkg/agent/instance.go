...
func initSessionStore(dir string) session.SessionStore {
	...
	if supabaseURL != "" && supabaseKey != "" {
		...
		if err != nil {
			logger.WarnCF("agent", "Supabase memory store init failed; falling back to local storage",
				map[string]any{"error": err.Error()})
		} else {
			logger.InfoCF("agent", "Using persistent Supabase memory store",
				map[string]any{"url": supabaseURL})
			return session.NewJSONLBackend(supaStore)
		}
	}
